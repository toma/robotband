...
void PianoRoll::overrideLightColor(int lightColor) {
    Serial.print("Overriding light color: ");
    Serial.println(lightColor);
    overriddenLightColor = lightColor;
}
...
unsigned char *PianoRoll::getStateSet() {
    ...
    if (instructionFile.available() != 0) {
        ...
        for (unsigned char i = 0; i <= 6; i++) {
            if (overriddenLightColor != -1) {
                pixelColor[i] = overriddenLightColor;
            } else {
...
void PianoRoll::stop() {
    ...
    if (overriddenLightColor == -1) {
        ...
        pixelColor[0] = overriddenLightColor;
        pixelColor[1] = overriddenLightColor;
        pixelColor[2] = overriddenLightColor;
        pixelColor[3] = overriddenLightColor;
        pixelColor[4] = overriddenLightColor;
        pixelColor[5] = overriddenLightColor;
        pixelColor[6] = overriddenLightColor;
    }
}
